...
import { structuredOk, structuredWithImage, toolError, type ToolResult } from "../utils/render.js";
...
export const getEsimStatusHandler = (http: HttpClient) =>
  async (args: { esim_id: string }): Promise<ToolResult> => {
    try {
...
export const topupEsimHandler = (http: HttpClient) =>
  async (args: { esim_id: string; topup_product_id?: string }): Promise<ToolResult> => {
    try {
